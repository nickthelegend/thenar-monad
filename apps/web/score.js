/* score.js — the scorer, in the browser.
 *
 * A mirror of packages/protocol/src/score.ts, kept identical because the score
 * is committed into the episode leaf: the browser that computes it and the
 * exporter that re-checks it must agree exactly. `score.test.mjs` holds them to
 * each other across randomised trajectories.
 */
export const WEIGHTS = { placement: 0.55, smoothness: 0.25, efficiency: 0.2 };
 ...
 */
export const CALIBRATION = { jerkFloor: 0.26, jerkCeil: 162, parSeconds: 11 };
 ...
 */
export function meanJerk(samples) {
 ...
 */
export function smoothnessOf(jerk, cal = CALIBRATION) {
 ...
 */
export function scoreTrajectory(traj, cal = CALIBRATION) {
 ...
 */
export function calibrate(episodes) {
 ...
 */
export function canonicalTrajectory(traj) {
